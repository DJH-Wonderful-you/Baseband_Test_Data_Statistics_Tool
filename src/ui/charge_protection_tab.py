...
from src.core.charge_protection_service import process_charge_protection_statistics
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        options_layout.addWidget(
            self._build_mode_card(
                title="单文件模式",
                description="仅处理 Excel（.xlsx/.xls），用于统计高温/低温充电保护测试数据。",
                button_text="执行单文件模式",
...
class ChargeProtectionTab(QWidget):
    BATCH_PACING_ENABLED_KEY = "charge_protection_tab/batch_pacing/enabled"
    BATCH_PACING_CHUNK_SIZE_KEY = "charge_protection_tab/batch_pacing/chunk_size"
    BATCH_PACING_WAIT_SECONDS_KEY = "charge_protection_tab/batch_pacing/wait_seconds"
    ...
    def _build_ui(self) -> None:
        ...
        upload_hint = QLabel("支持拖拽或选择文件/文件夹。充电保护测试仅处理 Excel（.xlsx/.xls）。")
    ...
    def _open_statistics_mode_dialog(self) -> None:
        dialog = ChargeProtectionModeDialog(self)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        if dialog.selected_mode == ChargeProtectionModeDialog.MODE_SINGLE:
    ...
    def _run_statistics(self) -> None:
        ...
        self._dispatch_statistics_task(
            ...
            task=process_charge_protection_statistics,
