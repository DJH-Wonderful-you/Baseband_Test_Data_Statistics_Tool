...
class AboutTab(QWidget):
    def __init__(self) -> None:
        ...
        usage = self._build_card(
            ...
            "5️⃣ 处理完成后，可点击“打开文件夹”直接打开当前输出目录，查看按模式生成的 Excel 结果",
        )
        ...
        self._entries = [
            {
                ...
                "detail": (
                    "【版本目标】\n"
                    "优化各统计界面的输出目录操作体验，避免“浏览”按钮再承担修改路径职责，统一改为直接打开当前输出目录。\n\n"
                    "【主要更新】\n"
                    "1. 输出目录按钮交互调整：\n"
                    "   - 充电测试、充电保护测试、分压采集测试、续航测试四个界面的按钮文案统一调整为“打开文件夹”。\n"
                    "   - 点击后直接打开当前输出路径对应的文件夹，不再弹出目录选择框，也不再修改输出路径文本。\n"
                    "2. 打开目录能力增强：\n"
                    "   - 当输出目录尚不存在时，系统会先自动创建目录，再调用系统文件管理器打开。\n"
                    "   - 当路径为空、路径无效或打开失败时，界面会给出明确提示。\n\n"
                    "【兼容说明】\n"
                    "本版本仅调整输出目录相关 UI 交互与版本信息，不改变既有统计逻辑、输出结构和各模式处理口径。"
                ),
            },
            {
                "version": "V1.5",
