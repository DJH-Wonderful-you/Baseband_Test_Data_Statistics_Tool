...
class AboutTab(QWidget):
    def __init__(self) -> None:
        ...
        positioning = self._build_card(
            ...
            "• 续航测试：支持“执行单Excel续航时长统计 / 执行单log续航时长统计 / 执行电量指示统计”三类统计任务（单log模式仅接收 .txt/.log，支持批量）\n"
        ...
        usage = self._build_card(
            ...
            "   - 充电测试：单文件模式 / 合并模式\n"
            "   - 续航测试：执行单Excel续航时长统计 / 执行单log续航时长统计（仅文本输入，支持批量） / 执行电量指示统计\n"
        ...
        notes = self._build_card(
            ...
            "• V1.2：新增“执行单log续航时长统计”能力增强（仅文本输入、支持批量）；单log图表按秒补齐电量持续区间；特殊文本处理迁移至单log分支并恢复 0:00:00 起始秒级时间；单log结果页隐藏日期列；单log防误触弹窗样式与其它控件统一。\n"
            "• V1.1：电量指示统计新增“续航时长”结果表（按需求 2.2.2 第（6）（7）点）；BUG修复：特殊无时间戳日志（MCU:[APP-I:bat]...）已确保按顺序读取全部电压/电量数据，不执行“首次电量去重”规则，分支返回原始全量点位。\n"
...
class UpdateLogTab(QWidget):
    def __init__(self) -> None:
        ...
        self._entries = [
            ...
            {
                "version": "V1.1",
                "title": "电量指示统计增强：新增续航时长统计与特殊日志全量读取",
                "time": "2026-03-07",
                "detail": (
                    "【版本目标】\n"
                    "补齐“电量指示数据统计”在续航时长输出上的需求闭环，并修正特殊文本日志解析规则与需求文档保持一致。\n\n"
                    "【主要更新】\n"
                    ...
                    "【兼容说明】\n"
                    "本版本不改变既有充电统计指标口径，仅增强续航-电量指示流程的输出能力与特殊日志处理一致性。"
                ),
            },
            {
                "version": "V1.0",
